...
1.  **Khối Nội dung `[BLOCK]`:**
    * Sử dụng `[BLOCK]` trên một dòng riêng để bắt đầu một đơn vị nội dung trực quan (ví dụ: một đoạn văn, một slide carousel, một gạch đầu dòng).
    * Mỗi `[BLOCK]` tương ứng với một lần xuống dòng trực quan lớn (tương đương thẻ `<p>` hoặc một slide).

2.  **Tiêu đề `[TITLE]...[/TITLE]`:**
    * Dùng để xác định tiêu đề chính của một `[BLOCK]`. Dòng này sẽ được hiển thị với font chữ lớn nhất.
    * Ví dụ: `[TITLE]3 Câu Hỏi Quan Trọng[/TITLE]`

3.  **Nhấn mạnh `[HIGHLIGHT]...[/HIGHLIGHT]`:**
    * Sử dụng để bao bọc các từ khóa hoặc cụm từ quan trọng cần in đậm.
...
2.  **Tái cấu trúc:** Chia `input_content` thành các đơn vị logic.
3.  **Áp dụng Cú pháp:** Bọc các đơn vị logic bằng `[BLOCK]`, `[TITLE]`, `[HIGHLIGHT]`...
...
5.  **Xuất kết quả:** Trả về một chuỗi văn bản duy nhất tuân thủ tất cả các quy tắc trên."""
    ...
    def _format_structured_content(self, content: str) -> str:
        """Convert structured tags to readable format - NEW METHOD"""
        ...
        for line in lines:
            ...
            # Handle [BLOCK] - start new paragraph
            if line == "[BLOCK]":
                if current_block:
                    formatted_lines.append("\n".join(current_block))
                    current_block = []
            ...
            # Skip [/BLOCK] - already handled by [BLOCK]
            if line == "[/BLOCK]":
                continue

            # Handle [TITLE]...[/TITLE] - make bold and prominent
            title_match = re.match(r"\[TITLE\](.*?)\[/TITLE\]", line)
            if title_match:
                title_text = title_match.group(1)
                current_block.append(f"🎯 **{title_text}**")
                continue

            # Handle [HIGHLIGHT]...[/HIGHLIGHT] - make bold
            line = re.sub(r"\[HIGHLIGHT\](.*?)\[/HIGHLIGHT\]", r"**\1**", line)

            # Handle [EMOJI] - keep as is but remove tag
            line = re.sub(r"\[EMOJI\]", "", line)

            # Handle [CTA]...[/CTA] - format prominently
            cta_match = re.match(r"\[CTA\](.*?)\[/CTA\]", line)
            if cta_match:
                cta_text = cta_match.group(1)
                current_block.append(f"\n👉 **{cta_text}**")
                continue

            # Handle [HASHTAGS]...[/HASHTAGS] - format hashtags
            hashtags_match = re.match(r"\[HASHTAGS\](.*?)\[/HASHTAGS\]", line)
            if hashtags_match:
        ...
        # Add any remaining content in current_block
        if current_block:
            formatted_lines.append("\n".join(current_block))
